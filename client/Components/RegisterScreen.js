...
export default function RegisterScreen({ navigation }) {
  ...
  const checkEmail = async (email) => {
    const response = await fetch(
      `http://192.168.0.26:8080/api/member?email=${email}`
    );
  ...
  const checkNickName = async (nickname) => {
    const response = await fetch(
      `http://192.168.0.26:8080/api/member?nickname=${encodeURI(
  ...
  const register = async () => {
    if (checkAll()) {
      ...
      if (response.ok) {
        ...
        console.log(data);
        await fetch(
          `http://192.168.0.26:8080/api/email/send?email=${data.email}&&nickname=${data.nickname}&&certified=${data.certified}`
        );
      } else {
